    ...
    const app = createApp({
        data() {
            return {
                ...
                // 对话历史侧边栏
                sidebarCollapsed: false,  // 默认显示对话侧边栏
                conversations: [],
        ...
        methods: {
            initScrollListener() {
                ...
                messagesArea.addEventListener('scroll', (e) => {
                    ...
                    // 延迟判断：如果用户停止滚动且在底部，恢复自动滚动
                    scrollTimer = setTimeout(() => {
                        const currentScrollTop = messagesArea.scrollTop;
                        const currentScrollHeight = messagesArea.scrollHeight;
                        const currentClientHeight = messagesArea.clientHeight;
                        const currentIsAtBottom = currentScrollHeight - currentScrollTop - currentClientHeight < 10;
                    ...
                    this.socket.on('update_action', (data) => {
                        ...
                        if (this.currentMessageIndex >= 0) {
                            ...
                            for (let action of message.actions) {
                                if (action.type === 'tool') {
                                    // 检查执行ID或准备ID
                                    if (action.tool.executionId === data.id || 
                                        action.tool.id === data.id || 
                                        action.id === data.preparing_id) {
            ...
            sendMessage() {
                if (!this.inputMessage.trim() || !this.isConnected || this.streamingMessage) {
                    return;
                }
